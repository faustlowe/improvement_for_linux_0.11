...
int copy_mem(int nr,struct task_struct * p)
{
	...
	if (copy_page_tables(old_page_dir,new_page_dir,kernel_data_base,kernel_data_base,kernel_data_limit)){	//kernel base is same for every process
		printk("no mem for kernel,free_page_tables:from copy_mem\n");
...
int copy_process(int nr,long ebp,long edi,long esi,long gs,long none,
		long cr3,long ebx,long ecx,long edx,
		...
		long eip,long cs,long eflags,long esp,long ss)
{
	...
	if (!p)
		return -EAGAIN;
	if (!(page_dir = (unsigned long *) get_free_page())){	//if there is no mem for dir,free p and exit
		free_page((long) p);
		return -EAGAIN;
	}
	task[nr] = p;
	*p = *current;	/* NOTE! this doesn't copy the supervisor stack */
	...
	p->tss.cr3 = (unsigned long) page_dir;					//we set cr3 for a liner address for each process
	if (last_task_used_math == current)
		__asm__("clts ; fnsave %0"::"m" (p->tss.i387));
	if (copy_mem(nr,p)) {
		task[nr] = NULL;
		free_page((long) p);
		free_page((long) page_dir);			//if copy_mem go wrong,remember free page_dir
		return -EAGAIN;
	}
	for (i=0; i<NR_OPEN;i++)
		if ((f=p->filp[i]))
